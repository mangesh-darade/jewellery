    ...
    public function permissions($id = NULL) {
        ...
        if ($this->form_validation->run() == TRUE) {
            $data = array('products-index' => $this->input->post('products-index'),
                ...
                'reports-sales-person-report' => $this->input->post('sales-person-report'),
                'reports-staff-report' => $this->input->post('staff-report'),
...
$data['sales_person']  = !empty($this->input->post('sales_person'))?$this->input->post('sales_person'):0;
 $data['signature']  = !empty($this->input->post('signature'))?$this->input->post('signature'):0;
   ...
   /**
    ...
    public function varient_action(){
        $this->form_validation->set_rules('form_action', lang("form_action"), 'required');
        if ($this->form_validation->run() == true) {
           if (!empty($_POST['val'])) {
               if ($this->input->post('form_action') == 'delete') {
                  $result =   $this->settings_model->deleteVariantBulk($_POST['val']);
                  if($result){
                     $this->session->set_flashdata('message', $this->lang->line("variant_deleted"));
                     redirect($_SERVER["HTTP_REFERER"]);
                  }else{
                     $this->session->set_flashdata('error', $this->lang->line("variant_not_deleted"));
                     redirect($_SERVER["HTTP_REFERER"]);
                  }
               }
           }else{
                $this->session->set_flashdata('error', $this->lang->line("no_variant_selected"));
                redirect($_SERVER["HTTP_REFERER"]);
           }
        } else {
             $this->session->set_flashdata('error', validation_errors());
        ...
        if ($this->form_validation->run() == TRUE) {
            if (isset($_FILES["userfile"])) {
                ...
                foreach ($final as $csv_ct) {
                        ...
                        if(trim($csv_ct['variant_group']) == 'size'){
                           $groupid = 1 ;
                        }else if(trim($csv_ct['variant_group']) == 'color'){
                           $groupid = 2 ;
                        }else{
                            $this->session->set_flashdata('error', 'group id is not valid only(size/color)');
                            redirect("system_settings/variants");
                        }
                        
                        if (!$this->settings_model->getVariantByGrpId(trim($csv_ct['name']),$groupid)) {
                            $data[] = array('name' => trim($csv_ct['name']),'group_id' =>  $groupid,);
                        } else {
                            $this->session->set_flashdata('error', 'variant  ' . $csv_ct['name'] . ' already exist');
                            redirect("system_settings/variants");
                        }
                    }
            }
        }
        if ($this->form_validation->run() == TRUE && !empty($data) && $this->settings_model->add_Varients($data)) {
            ...
            $this->data['userfile'] = array('name' => 'userfile', 'id' => 'userfile', 'type' => 'text', 'value' => $this->form_validation->set_value('userfile'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/import_variant', $this->data);
        }
    }
    public function Update_daily_rate() {
        $rates = $this->input->get('rates');
        foreach ($rates as $id => $rate) {
            $this->settings_model->update_rate($id, $rate);
        }

        echo json_encode(['status' => 'success', 'message' => 'Rates updated successfully!']);
    }
     public function jewellery_settings() {

        if ($rates = $this->input->get('rates')) {
             foreach ($rates as $id => $rate) {
            $this->settings_model->update_rate($id, $rate);
        }
        echo json_encode(['status' => 'success', 'message' => 'Rates updated successfully!']);
            ...
            $this->data['rates'] = $this->pos_model->get_latest_jewellery_rates();

            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('Jewellery_Settings')));
            $meta = array('page_title' => lang('Jewellery_Settings'), 'bc' => $bc);
            $this->page_construct('settings/jewellery_settings', $meta, $this->data);
        }
    }
}
