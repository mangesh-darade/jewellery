    ...
    public function add($quote_id = null) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($warehouse[$warehouse_id]->state_code != ''){
                $billers_id = $this->site->getBillerByWarehouseId($warehouse_id);//multiple billers
                // $this->pos_settings->default_biller;
    ...
    public function purchase_by_csv() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $supplier_state_code = $supplier_details->state_code != '' ? $supplier_details->state_code : NULL;
                // $warehouse = $this->site->getWarehouseByID($warehouse_id); 
                $warehouse = $this->site->getWarehouseID($warehouse_id); 
                    ...
                    foreach ($rows as $row) {
                        ...
                        if ($options) {
                            ...
                            if($add_quick_product == 1){
                                // $quantity = $opt->unit_quantity; // for set variant quantity(checking for add quick product)
                                $quantity = $quick_product_quantity; // for set variant quantity(checking for add quick product)
