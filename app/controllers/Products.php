    ...
    function getProducts($warehouse_id = NULL) {
        ...
        if ($warehouse_id) {
            ...
            $this->datatables->select(" sma_products.id as productid, {$this->db->dbprefix('products')}.image as image, "
                                    ...
                                    . " {$this->db->dbprefix('categories')}.name as cname, latest_cost.lcost as cost, price as price,"
                                    . " COALESCE(SUM(DISTINCT wp.quantity), 0) as quantity, {$this->db->dbprefix('units')}.name as unit, wp.rack as rack, {$this->db->dbprefix('products')}.storage_type,"
                                    . " is_featured", FALSE)
                    ->from('products')
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            for ($m = 0; $m < $s; $m++) {
                ...
                if ($variants = $this->products_model->getProductOptionsByName($pid,$option_name)) {
                    foreach ($variants as $option) {
                        $all_variants = $this->products_model->getProductOptions($pid);
                            $barcodes[] = [
                ...
                if ($items) {
                    foreach ($items as $item_key => $item) {
                        if ($row = $this->products_model->getProductByID($item->product_id)) {
                            ...
                            if ($variants = $this->products_model->getProductOptions($row->id, SIZE)) {
                            ...
                            if ($colors = $this->products_model->getProductOptions($row->id, COLOR)) {
                           ...
                           $pr[$ri] = array('id' => $ri , 'label' => $row->name . " (" . $row->code . ")", 'code' => $row->code, 'name' => $row->name,'style_code' => $row->article_code != null ? $row->article_code : "-",'category' => $category,'price' => $row->price, 'qty' => $item->quantity, 'variants' => $variants, 'selected_variants' => $selected_variants, 'colors' => $colors, 'selected_variants_color' => $selected_variants_color, 'batchno' => $item->batch_number);
                        }
                    }
            ...
            if ($product_id) {
                if ($product = $this->site->getProductByIDwithBatchAndWarehouse($product_id,$warehouse_ids)) {
                    $selected_variants = false;
                    $variants = $this->products_model->getProductOptionswithbatchAndWarehous($product->id,$warehouse_ids);
                    $pr = [];
                    ...
                    if (!empty($variants)) {
                        foreach ($variants as $variant) {
                            $pr[$variant->id] = [
                                ...
                                'price' => $variant->price,
                                'quantity' => $variant->warehouse_quantity,
                                'variants' => $variants,
            ...
            if ($this->input->post('type') == 'standard') {
                if ($product_variants = $this->products_model->getProductOptions($id)) {
                    foreach ($product_variants as $pv) {
                        $update_variants[] = array('id' => $this->input->post('variant_id_' . $pv->id), 'name' => $this->input->post('variant_name_' . $pv->id), 'cost' => $this->input->post('variant_cost_' . $pv->id),'mrp' =>$this->input->post('variant_mrp_' . $pv->id), 'price' => $this->input->post('variant_price_' . $pv->id), 'up_price' => $this->input->post('variant_upprice_' . $pv->id), 'variant_discount_on_mrp' => $this->input->post('variant_discount_' . $pv->id),'unit_quantity' => $this->input->post('unit_quantity_' . $pv->id), 'unit_weight' => $this->input->post('unit_weight_' . $pv->id));
                    }
                } else {
    ...
    function import_csv() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('name', 'code', 'divisionid', 'article_code', 'barcode_symbology', 'brand', 'category_code', 'unit', 'sale_unit', 'purchase_unit', 'cost', 'price', 'alert_quantity', 'tax_rate', 'tax_method', 'image', 'subcategory_code', 'variants', 'mrp', 'hsn_code', 'warehouse', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6', 'season_id');
                ...
                foreach ($final as $csv_pr) { 
                    $productcost = isset($csv_pr['cost']) ? (float) trim($csv_pr['cost']) : 0;
                    $productmrp = isset($csv_pr['mrp']) ? (float) trim($csv_pr['mrp']) : 0;
                    $productprice = isset($csv_pr['price']) ? (float) trim($csv_pr['price']) : 0;

                    if (empty($productcost) || empty($productmrp) || empty($productprice)) {
...
//                        if ($catd = $this->products_model->getCategoryByCode(trim($csv_pr['category_code']))) {
            ...
            $ikeys = array('code', 'divisionid', 'barcode_symbology', 'name', 'brand', 'category_id', 'unit', 'sale_unit', 'purchase_unit', 'cost', 'price', 'alert_quantity', 'tax_rate', 'tax_method', 'subcategory_id', 'variants', 'cf1', 'cf2', 'cf3', 'cf4', 'cf5', 'cf6', 'mrp', 'hsn_code', 'warehouse', 'article_code', 'up_items', 'food_type_id', 'up_price', 'available','season_id','discount_on_mrp','other_categories',);
            ...
            foreach (array_map(NULL, $pr_code, $divisionid, $bs, $pr_name, $pr_brand, $pr_cat, $pr_unit, $sale_units, $purcahse_units, $pr_cost, $pr_price, $pr_aq, $pr_tax, $tax_method, $pr_subcat, $pr_variants, $cf1, $cf2, $cf3, $cf4, $cf5, $cf6, $mrp, $hsn_code, $warehouse, $pr_article_code, $up_items, $food_type_id, $up_price, $available, $season_id) as $ikey => $value) {
                
                $price_val = isset($value[10]) ? floatval(trim($value[10])) : 0;
                $mrp_val   = isset($value[22]) ? floatval(trim($value[22])) : 0;
                if ($mrp_val > 0 && is_numeric($price_val) && is_numeric($mrp_val)) {
                    $discount = round((($mrp_val - $price_val) / $mrp_val) * 100,0) . '%';
                } else {
                    $discount = '0%';
                }
                        
                $value['discount_on_mrp'] = $discount;
                if ($this->Settings->other_category_for_product == 1) {
                $value['other_categories'] = implode(',', $categoryIds);
                }
                ...
                if (!empty($value[15])) { 
                    ...
                    foreach ($variants as $index => $variant) {
                        ...
                        $variant_discount_on_mrp = ($mrp_var > 0) ? round((($mrp_var - $price_var) / $mrp_var) * 100, 0) . '%' : '0%';
                        $product_with_variants[] = [
                        'product_code' => trim($value[0]), // Assuming 0 is 'code'
        ...
        if ($this->form_validation->run() == TRUE && $prs = $this->products_model->add_import_csv_products($items,$product_with_variants)) {
        ...
        if ($this->form_validation->run() == TRUE && $this->products_model->updateAdjustment($id, $data, $products)) {
            ...
            foreach ($inv_items as $item) {
                 ...
                 * */
                 ...
                 $pr[$ri] = ['id' => $c, 'item_id' => $row_id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'option_batches' => $current_batch, 'options' => $options];
                $c++;
            }
    ...
    function delete_adjustment($id = NULL) {
     ...
     function modal_view($id = NULL) {
       ...
       $comma_separated_cat_names = implode(', ', $cat_names);
        ...
        $this->data['options'] = $this->products_model->getProductOptionsWithWH($id,$NULL, $warehouse_ids);
                ...
                if ($this->input->post('form_action') == 'sync_quantity') {
                 ...
                 elseif ($this->input->post('form_action') == 'labels') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        if ($variants = $this->products_model->getProductOptionswithbatchAndWarehous_bulkAction($id, $warehouse_ids)) {
                            if (!empty($variants)) {
                                $i = 0;
                                foreach ($variants as $variant) {
                                    $selected_variants[$variant->id] = $variant->quantity > 0 ? 1 : 0;
                            ...
                            $selected_variants[$product->id] = $product->warehouse_quantity > 0 ? 1 : 0;
                            $pr[$product->id] = [
                                'id' => $product->id,
                                'label' => "{$product->name} ({$product->code})",
                                'code' => $product->code,
    ...
    public function qa_suggestions($warehouse_id = null) {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                $row->cost = $this->input->get('subtotal', true);
                $row->storage_type = $product->storage_type;
    ...
    function quick_add_product() {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($this->Settings->product_name_auto_generate) {
                    ...
                    // $size = ($this->input->post('attributesInput'))? strtoupper(substr($this->input->post('attributesInput'),0,2))  :'';
                    $color = ($this->input->post('AttrColor')) ? strtoupper(substr($this->input->post('AttrColor[0]'), 0, 3)) : '';
                ...
                if ($this->input->post('attributes')) {
                    ...
                    for ($r = 0; $r <= $a; $r++) {
                        if (isset($_POST['attr_name'][$r])) {
                            ...
                            $attr_eshop_mrp = $attr_eshop_price > $mrp ? $attr_eshop_price : $mrp;
                            $quantity = $_POST['attr_unit_quantity'][$r];
                ...
                // merge product code and product variant quantity
                if (!empty($result) && is_array($result) && isset($_POST['attr_unit_quantity'])) {
                    foreach ($result as $key => &$res) {
                        if (isset($_POST['attr_unit_quantity'][$key])) {
                            $res['quantity'] = $_POST['attr_unit_quantity'][$key];
                        }
                    }
                }
        ...
        if ($variant) {
            ...
            $this->data['variants'] = $this->products_model->getAllVariants1(SIZE);
        }

        $this->data['variants_color'] = $this->products_model->getAllVariants1(COLOR);
        $this->data['modal_js'] = $this->site->modal_js();
        $this->data['seasons'] = $this->site->get_all_season();
