        ...
        $this->db->select("DATE_FORMAT({$this->db->dbprefix('purchases')}.date, '%Y-%m-%d %T') as date, reference_no, {$this->db->dbprefix('warehouses')}.name as wname, supplier,IF(comp.gstn_no IS NULL or comp.gstn_no = '', '-', comp.gstn_no) as gstn_no,
           ...
           (SELECT  IF(item_tax IS NULL or item_tax = '', 0 , CONCAT(format(sum(ifnull(item_tax,0)),2)) )  FROM `sma_purchase_items` WHERE `sma_purchase_items`.`purchase_id` = `sma_purchases`.`id` ) as tax_amt,
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('V2', lang('Product Name'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('H' . $row, $this->sma->formatMoney($data_row->tax_able_amount));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $this->sma->formatMoney($data_row->tax_amt));
                    $this->excel->getActiveSheet()->getStyle('J' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('K' . $row, $this->sma->formatMoney($data_row->paid));
                    $this->excel->getActiveSheet()->getStyle('K' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('L' . $row, $this->sma->formatMoney($data_row->grand_total - $data_row->paid));
                    ...
                    $this->excel->getActiveSheet()->getStyle('M' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

                    $this->excel->getActiveSheet()->SetCellValue('N' . $row, $this->sma->formatMoney($CGST));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('P' . $row, $this->sma->formatMoney($SGST));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('R' . $row, $this->sma->formatMoney($IGST));
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('T' . $row, $this->sma->formatMoney($Vat->taxamount));
                    $this->excel->getActiveSheet()->getStyle('T' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('U' . $row, $this->sma->formatMoney($Cess->taxamount));
                    ...
                    $this->excel->getActiveSheet()->getStyle('V' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                ...
                if($sale_items == TRUE){
                    ...
                    foreach ($row['gst'] as $tax) {
                        ...
                        $td_taxes = '<td>'.number_format($tax['tax_rate'], 2).'%</td> 
                                        <td align="center">'.$this->sma->formatQuantity($tax['total_items']).'</td> 
     ...
     private function __sale_gst_report_download($rowData, $rowFooter) {
        ...
        if (!empty($rowData)) {
           ...
           // $this->excel->getActiveSheet()->getStyle("A" . $row . ":Y" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
            ...
            foreach ($rowData as $invoice_no => $inv) {
               ...
               $this->excel->getActiveSheet()->SetCellValue('AD' . $row, $inv[$cellData['AD']]);
                ...
                foreach ($inv['gst'] as $tax) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('S' . $row, $this->sma->formatMoney($total_discount));
                    $this->excel->getActiveSheet()->getStyle('S' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('T' . $row, $this->sma->formatMoney($total_amount));
                    $this->excel->getActiveSheet()->getStyle('T' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('U' . $row, $this->sma->formatMoney($total_tax_amount));
                    $this->excel->getActiveSheet()->getStyle('U' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            ...
            $this->excel->getActiveSheet()->SetCellValue('K' . $row, $this->sma->formatMoney($total_cgst));
            $this->excel->getActiveSheet()->getStyle('K' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('M' . $row, $this->sma->formatMoney($total_sgst));
            $this->excel->getActiveSheet()->getStyle('M' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('O' . $row, $this->sma->formatMoney($total_igst));
            $this->excel->getActiveSheet()->getStyle('O' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('P' . $row, $this->sma->formatMoney($total_tax_amount));
            $this->excel->getActiveSheet()->getStyle('P' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('Q' . $row, $this->sma->formatMoney($total_product_tax_amount));
            $this->excel->getActiveSheet()->getStyle('Q' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('R' . $row, $this->sma->formatMoney($total_invoice_tax_amount));
            $this->excel->getActiveSheet()->getStyle('R' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
