...
class Pos extends MY_Controller {
    ...
    public function index($sid = null) {
        ...
        if ($payment_method == 'cash' && $this->pos_settings->display_coinage == 1) {
              ...
              $collected_amount = preg_replace('/[^\d.]/', '', $this->input->post('deposited_amount'));
              $pending_currency_value = (int)preg_replace('/[.,]/', '', $this->input->post('pending_amount')); 
              $return_amount = (int) preg_replace('/[^\d.]/', '', $this->input->post('return_Amount'));

              $invoice_amounts = $this->input->post('invoice_amounts');
          
        if ($invoice_amounts < $collected_amount) {
            if ($pending_currency_value > 0) {
            $pending_amount = ($this->sma->formatDecimal($collected_amount) - $this->sma->formatDecimal($invoice_amounts));
        } else if($pending_currency_value == 0 || $pending_currency_value < 0 ) {
                        $pending_amount = $pending_currency_value;
                    }
        else {
            $pending_amount = $pending_currency_value;
              }
                    ...
                    if (isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
                        ...
                        if ($_POST['paid_by'][$r] == 'deposit') {
                            ...
                            if ($this->pos_settings->display_coinage == 1) {
                                if ($invoice_amounts > $pos_paid) {
                                    $amount =  $pos_paid;
                                    $pending_amount =$pos_paid- $invoice_amounts; 
                                }
                                if($selected_amounts > 0){  // Cust Pays more Gets Exact returned Bill Paid exact
                                    $pending_amount = 0;
                                }
                            }
                            $payment[] = array(
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if ($suspend) {
                ...
                if ($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
                    $sale_id = is_array($sale) && isset($sale['sale_id']) ? $sale['sale_id'] : null;

                    if (!empty($generated_tokens)) {
                        foreach ($generated_tokens as $token_entry) {
                            $update = ['sale_id' => $sale_id];

                            $where = [
                                'category_id' => $token_entry['category_id'],
                                'tokan'       => $token_entry['tokan'],
                                'kot_date'    => date('Y-m-d'),
                                'sale_id'     => null  // This is fine to ensure we update only unlinked records
                            ];

                        $this->pos_model->actionkotlog('Update', $update, $where);
                    }
                    }
...
public function close_register($user_id = null) {
    ...
    // backup function before variant MRP functionality
    ...
    public function ajaxproducts($category_id = null, $brand_id = null,$seasons_id) {
        ...
        if (!empty($products)) {
            ...
            foreach ($products as $product) {
                ...
                if ($Settings->theme != 'default' && $Settings->theme != 'newpos' && $Settings->theme != 'theme_three') {
   ...
   public function checkCustomerdetails() {
    ...
    public function saveCoinageTransactions($updateData,$currency_type = null, $coinage_id = null) {
     ...
     public function open_register() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->pos_model->openRegister($data, $data_coinage, $data_coinage_transactions)) {
                $this->session->set_flashdata('message', lang("welcome_to_pos"));
     ...
     public function update_register()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->pos_model->update_register($data, $data_coinage, $data_coinage_transactions,$Register_id)) {
        ...
        $this->data['transaction_types'] = $this->pos_model ->get_all_transaction_types();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
