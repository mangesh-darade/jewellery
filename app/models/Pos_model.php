...
//                    echo '<pre>';
 ...
 $printerSettings = $this->site->defaultPrinterOption($this->Settings->default_printer);
        ...
        if ($this->pos_settings->item_order == 0 || $this->pos_settings->item_order == 2) {
            $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, products.details as details, sale_items.category_id as category_id, categories.name as category_name, product_variants.price as variant_price')
           ...
           /* if ($this->pos_settings->display_category == 0)
            ...
            $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, sale_items.category_id as category_id, categories.name as category_name, products.details as details')
                    ->join('tax_rates', 'tax_rates.id=sale_items.tax_rate_id', 'left')
                    ->join('product_variants', 'product_variants.id=sale_items.option_id', 'left')
                    ->join('products', 'products.id=sale_items.product_id', 'left')
                    ->join('categories', 'categories.id=sale_items.category_id', 'left')
...
// print_r($sData);
// exit;
//             if ($sData['reference_no'] == $referene_no) {
    ...
    if ($q->num_rows() > 0) {
        foreach (($q->result()) as $row) {
            ...
            if ($row->expiry_date < $today && $row->status != 'expired') {
                ...
                $row->status = 'expired'; // Update in the returned object as well
            }
...
//                $this->db->group_by('date(date)');
  ...
  public function get_rate($metal_type, $metal_purity) {
    ...
    //     $this->db->join('sma_products p', 'r.metal_type = p.cf1');
    //     $this->db->where('p.type', 'Jewellery');
    //     $this->db->order_by('r.created_at', 'DESC');
    //     $this->db->group_by('r.metal_type, r.metal_purity'); // Optional: if you want 1 row per metal+purity
    //     $query = $this->db->get();
    //     return $query->result();
    // }
    public function get_latest_jewellery_rates() {
