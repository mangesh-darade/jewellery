...
class Products_model extends CI_Model {
   ...
   public function get_category_names_by_ids($ids) {
    ...
    public function getProductOptionsAdjustment($pid, $warehouse_id = null) {
        $user = $this->site->getUser();
   ...
   public function getProductOptionsWithWH($pid, $GroupId='',$warehouse_ids)
    ...
    public function getAllWarehousesWithPQ($product_id, $warehouse_ids) {
        ...
        $this->db->select('' . $this->db->dbprefix('warehouses') . '.*, ' . $this->db->dbprefix('warehouses_products') . '.quantity, ' . $this->db->dbprefix('warehouses_products') . '.rack')
                ...
                if (!empty($warehouse_ids) && is_array($warehouse_ids)) {
                    $this->db->where_in('warehouses_products.warehouse_id', $warehouse_ids);
                }
    ...
    public function add_import_csv_products($products = array(),$product_with_variants = array()) {
        if (!empty($products)) {
            foreach ($products as $product) {
    ...
    public function getProductQuantity_by_warehouse($product_id, $warehouse) {
        // Convert warehouse(s) into array
...
// Get warehouseProduct List
    ...
    public function productAddQuick($productsdata,$product_attributes =[]){
        ...
        // $p_color = ($this->input->post('AttrColor')) ? strtoupper(substr($this->input->post('AttrColor[0]'), 0, 3)) : '';
        //  if(!empty($this->input->post('AttrColor'))){
        //         $color = [
        //             'product_id' => $product_id,
        //             'name' => $p_color,
        //             'group_id' => '2',
    ...
    public function getProductOptionswithbatchAndWarehous($pid, $warehouseId) {
        $this->db->select('product_variants.*, product_batches.batch_no, warehouses_products_variants.quantity AS warehouse_quantity');
        $this->db->from('product_variants');
    ...
    public function getProductOptionswithbatchAndWarehous_bulkAction($pid, $warehouseId) {
