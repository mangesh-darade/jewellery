        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group all" style="width: 93%;">
                        ...
                        <div class="well well-sm">
                            ...
                            <table class="table table-bordered table-condensed table-striped" style="margin-top: 10px;">
                                <div class="table-responsive">
                                    <thead>
                                        <tr class="active">
                                            <th style="width: 33px;"><?= lang('Pri ma ry') ?></th>
                                            <th style="width: 18rem;"><?= lang('Varient Name') ?></th>
                                            <th style="width: 100px;"><?= lang('Cost') ?></th>
                                            <!-- <th class="col-xs-2"><?= lang('Price_Addition') ?></th> -->
                                            <th style="width: 100px;"><?= lang('MRP') ?></th>
                                            <th style="width: 100px;"><?= lang('Price') ?></th>
                                            <th style="width: 65px;"><?= lang('Dis') ?></th>

                                            <?php if ($Settings->pos_type == 'restaurant') { ?>
                                            <th class="col-xs-1.5"><?= lang('Urbanpiper_Price_Addition') ?></th>
                                            <?php } ?>
                                            <th style="width: 80px;"><?= lang('Unit_Qty') ?></th>
                                            <th style="width: 70px;"><?= lang('Unit_Wgt(In KG)') ?></th>
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        foreach ($product_variants as $pv) {
                                            ...
                                            . '<td><input type="number" step="0.01" name="variant_cost_' . $pv->id . '" value="' . rtrim(rtrim((string)$pv->cost, '0'), '.') . '" class="form-control attr-input variant_cost"></td>'
                                            . '<td><input type="number" step="0.01" name="variant_mrp_' . $pv->id . '" value="' . rtrim(rtrim((string)$pv->mrp, '0'), '.') . '" class="form-control attr-input variant_mrp"></td>'
                                            . '<td><input type="number" step="0.01" name="variant_price_' . $pv->id . '" value="' . rtrim(rtrim((string)$pv->price, '0'), '.') . '" class="form-control attr-input variant_price"></td>';
                                            $discount_value = $pv->variant_discount_on_mrp;
                                                ...
                                                echo '<td><input type="text" name="variant_discount_' . $pv->id . '" value="' . $discount_value . '" class="form-control attr-input variant_discount "></td>';
                            ...
                            <div class="form-group">
                                <input type="checkbox" class="checkbox" name="attributes" id="attributes"
                                    <?= $this->input->post('attributes') ? 'checked="checked"' : ''; ?>>
        ...
        for (var i in validAttrs) {
            ...
            if (attrTrimmed !== '' && $.inArray(attrTrimmed, attr_arr_unique) === -1) {
                ...
                $('#attrTable').show().append(
                    '<tr class="attr">' +
                    '<td><input type="hidden" class="attr_name" name="attr_name[]" value="' + attrs[
                        i] + '"><span>' + attrs[i] + '</span></td>' +
                    ...
                    '<input type="number" class="form-control variantmrp text-right" id="variantmrp"name="attr_mrp[]" value="0"  oninput="updateHiddenInput(this, \'attr_mrp\')">' +
                    '</td>' +
...
$(document).ready(function() {
    ...
    $('#mrp').on('change', function() {
        ...
        if (!isNaN(mrpValue) && mrpValue > 0) {
            ...
            $('#discount_on_mrp').val('0%'); // Reset discount when MRP is changed
        } else {
            $('#price').val('0');
        }
    ...
    $(document).on('input', '.cost, .variantmrp, .variantdiscount, .variantprice, .unit_quantity,.unit_weight',
        function() {
            ...
            // Remove the percentage sign from the discount if it exists
...
$(document).on('change', '.variant_cost, .variant_mrp, .variant_price, .variant_discount, .unit_quantity, .unit_weight',
    function() {
        var row = $(this).closest('tr');
        ...
        var cost = parseFloat(costInput.val().replace(',', '')) || 0;
        var mrp = parseFloat(mrpInput.val().replace(',', '')) || 0;
        var price = parseFloat(priceInput.val().replace(',', '')) || 0;
        var quantity = parseFloat(quantityInput.val().replace(',', '')) || 0;
        var weight = parseFloat(weightInput.val().replace(',', '')) || 0;
        var discountText = discountInput.val().trim();
        var discount = 0;
        ...
        if ($(this).hasClass('variant_discount')) {
            ...
            if (isPercentage) {
                ...
                priceInput.val(formatNumber(price)); // Save price rounded to 2 decimal places
        ...
        if ($(this).hasClass('variant_price')) {
            ...
            price = roundToTwo(price); // Round price to 2 decimal places
            priceInput.val(formatNumber(price)); // Ensure price is shown with 2 decimals
        }
        priceInput.val(addCommas(priceInput.val()));
    });

function formatNumber(value) {
    if (isNaN(value)) return '0.00';
    value = value.toFixed(2); // Make sure to round it to two decimal places
