...
//                                      
                ...
                <div class="">
                       <div class="well-sm" style="<?= (($default_printer->footer_align == 'center') ? '' : 'float:' . $default_printer->footer_align . '; width: 69%; ') ?>text-align: <?= (isset($default_printer->footer_align) && $default_printer->footer_align != '') ? $default_printer->footer_align : 'center' ?>; font-size: 9pt;">
...
if ($_SERVER['HTTP_REFERER'] == base_url('pos')) {
    ...
    foreach ($rows as $rowItems) { 
        if ($pos_settings->print_all_category || in_array($rowItems->category_id, $category_Array)) {
...
<div class="page-break" id="order_print_bill_<?= $key ?>" style="display:none">
    <style>
        ...
        .no-border { border: 0; } 
        #orderTable_<?=$key?>>tbody>tr>td,#orderTable_<?=$key?>>tbody>tr>th {
            border: 1px solid; padding: 5px 2px;
        }
...
if (isset($category_tokens[$token_key])) {
    $token_no = $category_tokens[$token_key];
} else {
    $token_no = 'N/A';
}
...
?>
<span>Token No: <?= $token_no ?>, <?= date('d/m/y h:i A', strtotime($inv->date)); ?></span><br/>
    </div> 
