...
<head>
    ...
    td.text-center.item_master_qty {
    ...
    <link rel="stylesheet" href="<?= $assets ?>pos/css/<?= $Settings->theme ?>.css" type="text/css" />
    <?php }?>
    <?php if ($Settings->theme == 'theme_four') { ?>
    <style>
    #ajaxproducts img {
   ...
   input#add_item_qr {
    ...
    <?php }  ?>
    <?php
     if ($Settings->theme == 'theme_four' && $pos_settings->display_coinage == 1) { ?>
            ...
            <div class="container">
                ...
                <div class="header-nav">
                    ...
                    <ul class="nav navbar-nav pull-right">
                        ...
                        <li class="dropdown hidden-sm">
                            <a class="Settingset" id="pos_setting" title="<?= lang('settings') ?>"
                                data-placement="bottom" href="<?= site_url('pos/settings') ?>">
                                                ...
                                                <div class="col-xs-4 padding0">
                                                    <div class="btn-group-vertical btn-block">
                                                        ...
                                                        <button type="button" id="suspend"
                                                            ...
                                                            KOT </button>
                                                        <?php } elseif ($Settings->theme == 'theme_four' ||  $Settings->theme == 'theme_three') { ?>
                                ...
                                <div class="quick-menu">
                                    ...
                                    <div id="proContainer">
                                        ...
                                        <div class="cat-outer pd-set">
                                            <div class="hscroll dragscroll" id="category-container">
                                                <?php foreach ($categories as $category) {
                                                    ...
                                                    $defaultCategory =$this->data["pos_settings"]->default_category;
                                                ...
                                                echo "<div class='inline-item'>
                                                            <button id='category-" . $category->id . "' 
                                                                    ...
                                                                    onclick=\"toggleCategoryView('category-" . $category->id . "', $hasSubcategories)\" 
                                                                    style=\"background-image: $backgroundImage;
                                                                           background-color: " . ($category->id == $defaultCategory ? '#FFFFFF' : '#008E80') . "; 
                                                                           color: " . ($category->id == $defaultCategory ? '#000000' : '#FFFFFF') . "; 
                                                                            border: " . ($category->id == $defaultCategory ? '1px solid #006400' : 'none') . ";
                                                                           background-size: auto;
                                                                           background-repeat: no-repeat;
                                                                           background-position: top right;
                                                                           border-top-right-radius: 1.4rem;
                                        ...
                                        function toggleCategoryView(categoryId, hasSubcategories) {
                                            ...
                                            // Reset previously active styles
                                            if (activeCategory) {
                                            ...
                                            // Use class instead of inline color
                                            clickedText.classList.remove('text-white');
                                            clickedText.classList.add('text-dark');

                                            // Update localStorage AFTER checks
                                        ...
                                        function resetActiveCategoryStyles() {
                                            const prevButton = document.getElementById(activeCategory);
                                            const prevText = document.getElementById('text-' + activeCategory);

                                            if (prevButton && prevText) {
                                                prevButton.style.backgroundColor = '#008e80';
                                                prevButton.style.border = 'none';

                                                prevText.classList.remove('text-dark');
                                                prevText.classList.add('text-white');
                                            }

                                            if (activeIcon && activeIcon.parentElement) {
                        ...
                        <table class="table table-bordered table-striped">
                            <tr>
                                ...
                                <td style="display: none;">
                                    ...
                                    <input type="hidden" class="form-control kb-pad inputvalcheck selling"
                                        id="pdiscount">
                                </td>
                                <?php } ?>
                                ...
                                <td>
                                    ...
                                    <input type="text" class="form-control kb-pad inputvalcheck selling"
                                        id="mrpdiscount">
                                </td>
                            </tr>
                        </table>
        ...
        $("#add_item").autocomplete({
            ...
            response: function(event, ui) {
                if ($(this).val().length >= 16 && ui.content[0].id == 0) {
                    ...
                    if (ui.content[0].Product_type === 'Bundle' && ui.content[0].is_out_of_stock ===
                        1) {
...
?>
                    ...
                    if (data !== null) {
                        ...
                        if (data.row.quantity == null || parseFloat(data.row.quantity) <=
                            0) {
                            bootbox.alert('<?= lang('no_match_found') ?>');
                            $('#modal-loading').hide();
                            return false;
        ...
        $(document).on('click', '.category', function() {
            ...
            $.ajax({
                ...
                //     nav_pointer();
                // });
                console.log("ocat_id", ocat_id);
                if (ocat_id) {
                    let element = document.getElementById(`category-${ocat_id}`);

                    element.children[0].setAttribute("style", "color:white");
                    $('#category-' + ocat_id).removeClass('active');
                    $('#category-' + ocat_id).css({
                        'background-color': '#008E80', // Default color for non-default category
                        'color': '#FFFFFF',
                        'border': 'none',
                    });
                }
                ...
                success: function(data) {
                    ...
                    if (response && Array.isArray(response.bills)) {
                        ...
                        if (created_by.includes(userId)) {
                            const matchedBill = response.bills.find(bill => bill
                                .created_by === userId);
                            if (matchedBill) {
                                $('#checkbox_' + matchedBill.id).prop('checked', false)
                                    .prop('disabled', true);
                            }
                        }
                    }
...
$(document).ready(function() {
    $(".final-submit-btn").click(function() {
        var depositedAmount = $('#depositedAmount').text(); //// Collected amount   
        var pendingAmount = parseInt($('#pendingValues').val()) ||
            0; ///////// Pending Amount
        //    var pendingAmounts = parseInt($("#pendingValue").text()) || 0; \\/ 
        var invoice_amounts = parseInt($("#amount_1").val()) ||
            0; ///////// invoice Amount
        var selected_amounts = $('#selectedValue').text(); //// selected amount   ts);
        $('#deposited_amount').val(depositedAmount);
        $('#pending_amount').val(pendingAmount);
        $('#invoice_amounts').val(invoice_amounts);
        $('#selected_amounts').val(selected_amounts);
