        ...
        success: function (data) {
            ...
            var table = $('#PrData1').DataTable({
                ...
                "aoColumns":  [
                    ...
                    null,        // 1: Name
                    null,        // 2: PV Name
                    null,        // 3: Category Name (optional)
                    { "mRender": formatQuantity, "bSearchable": false },  // 4
                    { "mRender": currencyFormat, "bSearchable": false },  // 5
                    { "mRender": formatQuantity, "bSearchable": false },  // 6
                    { "mRender": currencyFormat, "bSearchable": false },  // 7
                    { "mRender": currencyFormat, "bSearchable": false },  // 8
                    { "mRender": formatQuantity, "bSearchable": false },  // 9
                    { "mRender": currencyFormat, "bSearchable": false }   // 10
                ] ,
                "footerCallback": function (row, data, start, end, display) {
                    var api = this.api();
                    var totalPurchaseQty = api.column(4, { page: 'current' }).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(String(b).replace(/,/g, ''));
                    }, 0);
                    var totalSoldQty = api.column(6, { page: 'current' }).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(String(b).replace(/,/g, ''));
                    }, 0);
                    var totalPurchased = api.column(5, { page: 'current' }).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(String(b).replace(/,/g, ''));
                    }, 0);
                    var totalSold = api.column(7, { page: 'current' }).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(String(b).replace(/,/g, ''));
                    }, 0);
                    var totalProfit = api.column(8, { page: 'current' }).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(String(b).replace(/,/g, ''));
                    }, 0);
                    var totalStockQty = api.column(9, { page: 'current' }).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(String(b).replace(/,/g, ''));
                    }, 0);
                    var totalStockAmount = api.column(10, { page: 'current' }).data().reduce(function (a, b) {
                        return parseFloat(a) + parseFloat(String(b).replace(/,/g, ''));
                    }, 0);
                    $(api.column(4).footer()).html(formatQuantity(totalPurchaseQty));
                    $(api.column(5).footer()).html(currencyFormat(totalPurchased));
                    $(api.column(6).footer()).html(formatQuantity(totalSoldQty));
                    $(api.column(7).footer()).html(currencyFormat(totalSold));
                    $(api.column(8).footer()).html(currencyFormat(totalProfit));
                    $(api.column(9).footer()).html(formatQuantity(totalStockQty));
                    $(api.column(10).footer()).html(currencyFormat(totalStockAmount));
                ...
                <!-- <div class="table-responsive">
                    <table id="PrRData"
                        ...
                        <tr class="active">
                            ...
                            <th style="width: 97.7778px;padding: 17px 0px 0px 0px;"><?= lang("stock_in_hand"); ?></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
