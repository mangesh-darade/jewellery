...
// save and load the fields in and/or from localStorage
    ...
    $(document).on('click', '.edit', function () {
        ...
        var qty = row.children().children('.rquantity').val(),
            ...
            unit_price = formatDecimal(row.children().children('.quickchange').val()),
                        ...
                        $.each(optionArr, function (k, opt) {
                            ...
                            if (this_options.id == opt) {
                                if (this_options.price != 0 && this_options.price != '' && this_options.price != null) {
                                    if (manualedit == 1) { // Manual Price Edit
                                        item_price = formatDecimal(parseFloat(item.row.price));
...
if(batch_id){
            ...
            var pr_tax_val = 0, pr_tax_rate = 0, tax_rate = 0;
            // if (site.settings.theme == 'default' || site.settings.theme == 'newpos') { // product level tax
            ...
            tr_html += '<input name="cat_id[]" type="hidden"  value="' + item.row.category_id + '">';
       ...
       // tr_html += item.row.unit_lable + '</td>';
        ...
        if (site.settings.product_weight == 1) {
            ...
            tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + unit_price + '"><input class="form-control input-sm text-right quickchange" name="unit_price[]" type="text" id="unit_price_' + row_no + '" value="' + formatDecimal(unit_price) + '"></td>'
            // tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(unit_price) + '</span>'; // existing
            ...
            if ((site.settings.product_discount == 1 && allow_discount == 1) || item_discount) {
                ...
                tr_html += '<td class="text-right"><input class="form-control input-sm rdiscount quickchange"  name="product_discount[]" type="text" id="discount_' + row_no + '" value="' + item_ds + '"><span class="text-right sdiscount text-danger" id="sdiscount_' + row_no + '">'  + (parseFloat(item_ds) != '' ? '(' + item_ds + ')' : '') + '</br>' + formatMoney(0 - (item_discount * item_qty)) + '</span></td>';
            }
            ...
            if (site.settings.tax1 == 1) {
                ...
                tr_html += '<td class="text-right"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + tax_rate + '"><span class="text-right sproduct_tax" id="sproduct_tax_' + row_no + '">' + (parseFloat(pr_tax_rate) != 0 ? '(' + pr_tax_rate + ')' : '') + ' ' + formatMoney(pr_tax_val * item_qty) + '</span></td>';
