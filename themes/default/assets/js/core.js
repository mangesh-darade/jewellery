...
$(document).ready(function () {
    ...
    $(document).on('click', '.po-delete2', function (e) {
        ...
        $.ajax({type: "get", url: link,
            success: function (data) {
                console.log(data);
                if (data.includes("Sale/Purchase/Transfer/Quotation has been created against this product")) {
                    addAlert(data, 'warning');
                } else {
                    // Otherwise, assume deletion was successful
                    $('#' + sp[1]).remove();
                    addAlert(data, 'success');
                }
            },
...
function pqFormat1(x) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Match format: name (subtotal) - quantity
        let match = line.match(/^(.*)\s+\(([\d.]+)\)\s+-\s+([\d.]+)$/);

        if (match) {
            let name = match[1];
            let subtotal = formatQuantity2(match[2]);  // Apply formatting
            let quantity = formatQuantity2(match[3]);  // Apply formatting

            formatted += `${name} (${subtotal}) - ${quantity}<br>`;
        } else {
            // Fallback: show original line if pattern doesn't match
            formatted += line + '<br>';
        }
    }
